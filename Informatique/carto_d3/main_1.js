...
//	 {
//         if (pistes[i]==id) return
//         }
//       }

function addPlot(id,lon,lat) {
        console.log("Ajout plot id=%s lon=%s lat=%s ",id,lon,lat);
        const found=pistes.find(element => element.id==id)
   	if (found!=undefined)
	{
         console.log("Mise a jour Piste");
         pistes.push({'id' : id,'lon': lon,'lat' : lat, 'r': 4      });
        }
        else
	{
         console.log("Nouvelle Piste");
         pistes.push({'id' : id,'lon': lon,'lat' : lat, 'r': 5   });
        }
        console.log(found);
    }
